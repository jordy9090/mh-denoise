...
def question_id(row, fallback_i):
    ...
    return f"question_text:{normalize_question(question_text(row)) or fallback_i}"
...
def load_excluded_questions(path):
    if not path:
        return set()
    return {normalize_question(row.get("question", "")) for row in read_jsonl(path) if row.get("question")}


def valid_candidate(row, min_words, max_words, excluded_topics, excluded_questions):
    answer = clean_text(first_present(row, ANSWER_TEXT_COLS))
    qtext = question_text(row)
    if not answer or not qtext:
        return False
    ...
    if norm_topic(topic_value(row)) in excluded_topics:
        return False
    if normalize_question(qtext) in excluded_questions:
        return False
    return True
...
def build_output(row, out_i):
    ...
    record = {
        "id": f"counselchat_candidate_{out_i:04d}",
        "question_id": qid,
        "questionID": qid,
        ...
        "views": to_int(first_present(row, VIEWS_COLS, 0)),
        "questionTitle": clean_text(first_present(row, QUESTION_TITLE_COLS)),
        "questionText": clean_text(first_present(row, QUESTION_TEXT_COLS)),
    }
    ...
    for out_key, cols in [
        ...
        value = clean_text(first_present(row, cols))
        if value:
...
def main():
    ...
    parser.add_argument("--n_candidates", type=int, default=800)
    parser.add_argument("--output", default="data/raw/counselchat_safe_candidates.jsonl")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--shuffle", action="store_true")
    parser.add_argument("--min_answer_words", type=int, default=20)
    ...
    parser.add_argument("--exclude_questions_jsonl", default="")
    ...
    excluded_questions = load_excluded_questions(args.exclude_questions_jsonl)

    by_question = defaultdict(list)
    for i, row in enumerate(rows):
        if valid_candidate(row, args.min_answer_words, args.max_answer_words, excluded_topics, excluded_questions):
    ...
    selected = selected[: args.n_candidates]
    ...
    print("excluded question count:", len(excluded_questions))
    print("candidates after filters:", len(by_question))
    print("selected candidates:", len(out_rows))
